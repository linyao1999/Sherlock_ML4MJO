...
plotflg = os.environ.get("plotflg", "false").lower() == "true"
...
if not os.path.exists(save_dir / f"contribution_data_{exp_name}_exp{exp_num}.npz"):
    ...
    def evaluate_channel_ablation(model, cached_data, layer_attr, channel_idx, total_samples, output_size):
        ...
        if layer_attr == 'cnn_last':
            target_layer = model.cnn.network[-1]
        else:
    ...
    if model_name == 'CNN_MLP':
        ablation_targets = {
            "cnn_last": config["model"]["cnn"]["channels_list"][-1],
        }
    else:
        ablation_targets = {
    ...
    for layer_name, num_channels in ablation_targets.items():
        ...
        for c in range(num_channels):
            ...
            diff = np.abs(ablated_mse - baseline_mse)  # Positive = channel was useful
            layer_diffs.append(diff)

        print()  # Newline after \r progress
        results[layer_name] = np.array(layer_diffs)

    # Save raw results PLUS the baseline_mse
    np.savez(
        save_dir / f"contribution_data_{exp_name}_exp{exp_num}.npz", 
    ...
    print(f"\nFound existing contribution data. Loading from {file_path}...")
    
    # Load the compressed arrays
    loaded_data = np.load(file_path)
    
    # Reconstruct the variables needed for plotting
    if model_name == 'CNN_MLP':
        results = {
            "cnn_last": loaded_data["cnn_last"]
        }
    else:
        results = {
            "hid2": loaded_data["hid2"],
            "hid6": loaded_data["hid6"]
        }
    
    # .item() extracts the float value from the 0-d numpy array created by np.savez
...
if plotflg:
    ...
    if model_name == 'CNN_MLP':
        all_pct_change = (results['cnn_last'] / baseline_mse) * 100
    else:
    ...
    if model_name == 'UNet_A':
        plt.figure(figsize=(8.5, 6.5))
        idx_hid2 = np.arange(len(pct_hid2))
        idx_hid6 = np.arange(len(pct_hid2), len(all_pct_change))
        plt.plot(idx_hid2, pct_hid2, 'o', color='tab:blue', markersize=8)
        plt.plot(idx_hid6, pct_hid6, 'o', color='tab:orange', markersize=8)
