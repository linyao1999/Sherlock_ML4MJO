...
plot_save_path = save_dir / f"spectra_16exps_{exp_name}_{trial_tag}.png"
...
if "Original Input_mean" in flat_agg_data:
    ...
    ax.plot(wavenumbers, mean_val[1:21], color='black', linestyle='-', linewidth=2)
    ax.fill_between(wavenumbers, min_val[1:21], max_val[1:21], color='black', alpha=0.1)
...
for idx, key in enumerate(layer_keys):
    ...
    # Plot Mean line
    ax.plot(wavenumbers, mean_val[1:21], color=color, linewidth=2, alpha=1.0)
    # Plot Max/Min spread
    ax.fill_between(wavenumbers, min_val[1:21], max_val[1:21], color=color, alpha=0.2)
