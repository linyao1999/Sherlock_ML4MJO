...
# ======================================================================
print("\n" + "="*50)
print("=== PART 1: CALCULATING WINTER (DJF) METRICS ===")
print("="*50)
...
for trial_rank in TOP_TRIALS:
    ...
    for period_name, dates in PERIODS.items():
        # NOTE: Updated filename for winter metrics
        out_file = save_dir / f"metrics_{exp_name}_{trial_tag}_{period_name}_winter.npz"
        if os.path.exists(out_file):
            print(f"[{trial_tag}] Metrics for {period_name} winter already calculated. Skipping.")
            continue

        print(f"\n--- Evaluating Trial {trial_rank} | Period: {period_name.upper()} WINTER ({dates['start']} to {dates['end']}) ---")
        ...
        for i, fn in enumerate(fn_list):
            print(f"  -> Processing member {i+1}/{len(fn_list)} for {period_name} (Winter)...")
            if multi_lead:
                bcc, rmse = mjo.get_skill_one_all_leads_winter(
                ...
                bcc, rmse = mjo.get_skill_one_winter(
                    mjo_ind=output_var_name,
                    fn=fn,
                    datesta=dates["start"],
                    dateend=dates["end"],
            ...
            # Append None if no valid winter days were found for this file
            if bcc is not None:
                ind_bcc_list.append(bcc)
                ind_rmse_list.append(rmse)

        # Calculate Ensemble Mean Skill
        print(f"  -> Computing ensemble mean for {period_name} (Winter)...")
        if multi_lead:
            ens_bcc, ens_rmse = mjo.get_skill_all_leads_ensemble_mean_winter(
                fn_list=fn_list,
                mjo_ind=output_var_name,
                datesta=dates["start"],
                dateend=dates["end"],
            ...
            ens_bcc, ens_rmse = mjo.get_skill_ensemble_mean_winter(
                fn_list=fn_list,
                datesta=dates["start"],
                dateend=dates["end"],
                leadmjo=lead,
        ...
        if ens_bcc is not None:
            ...
            print(f"Saved {period_name.upper()} WINTER metrics to: {out_file}")
        else:
            print(f"  [Warning] No valid data matching winter filters for {period_name}. Skipping save.")

print("\nAll winter evaluations complete!")
...
if multi_lead:
    ...
    for period_name in PERIODS.keys():
        print(f"Generating winter plots for: {period_name.upper()}...")
        
        fig_bcc, ax_bcc = plt.subplots(1, 1, figsize=(8.5, 6.5))
        fig_rmse, ax_rmse = plt.subplots(1, 1, figsize=(8.5, 6.5))

        for i, trial in enumerate(TOP_TRIALS):
            data_path = save_dir / f"metrics_{exp_name}_t{trial}_{period_name}_winter.npz"
        ...
        ax_bcc.set_title(f'Winter (DJF) BCC Skill Comparison ({period_name.upper()})')
        ax_bcc.legend(loc='upper right', fontsize=14)
        ax_bcc.grid(True, linestyle=':', alpha=0.6)

        fig_bcc.tight_layout()
        bcc_save_path = save_dir / f"plot_bcc_comparison_{exp_name}_{period_name}_winter.png"
        ...
        ax_rmse.set_title(f'Winter (DJF) RMSE Skill Comparison ({period_name.upper()})')
        ax_rmse.legend(loc='upper left', fontsize=14)
        ax_rmse.grid(True, linestyle=':', alpha=0.6)

        fig_rmse.tight_layout()
        rmse_save_path = save_dir / f"plot_rmse_comparison_{exp_name}_{period_name}_winter.png"
    ...
    print("All winter plots generated and saved successfully!")
